// Credit to: WebCoder49 for the below code
...
export function handleTabKey(
  ...
  if (
    ...
    const before_selection = code.slice(0, input_element.selectionStart);
    const after_selection = code.slice(input_element.selectionEnd);
    const cursor_pos = input_element.selectionEnd + 1;
    const tab = '\t';
    input_element.value = before_selection + tab + after_selection;
